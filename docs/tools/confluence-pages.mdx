...
| `content_format` | `string` | No | (Optional) Explicit page body format override. Options: 'markdown', 'storage', or 'atlas_doc_format'. When provided, this overrides `convert_to_markdown`. |
**Example:**
...
<Tip>
Content is returned in Markdown format by default. Set `convert_to_markdown` to `false` to retrieve exact Confluence storage XHTML when macro preservation matters.
Use `content_format: "atlas_doc_format"` on Confluence Cloud pages when you need smart-link and datasource metadata preserved for round-tripping.
</Tip>
...
| `content` | `string` | Yes | The content of the page. Format depends on content_format parameter. Can be Markdown (default), wiki markup, storage format, or atlas_doc_format JSON |
| `parent_id` | `string` | No | (Optional) parent page ID. If provided, this page will be created as a child of the specified page |
| `content_format` | `string` | No | (Optional) The format of the content parameter. Options: 'markdown' (default), 'wiki', 'storage', or 'atlas_doc_format'. Wiki format uses Confluence wiki markup syntax |
