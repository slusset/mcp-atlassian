...
| `content_format` | `string` | No | (Optional) Explicit page body format override. Options: 'markdown', 'storage', or 'atlas_doc_format'. Use `storage` to preserve Confluence macro/layout metadata and `atlas_doc_format` for Confluence Cloud ADF pages when smart-link or datasource display metadata must round-trip unchanged. When provided, this overrides `convert_to_markdown`. |
**Example:**
...
Use `content_format: "atlas_doc_format"` on Confluence Cloud pages when you need ADF-native smart-link, datasource, or embedded Jira display metadata preserved for round-tripping.
</Tip>
...
| `content` | `string` | Yes | The content of the page. Format depends on content_format parameter. Can be Markdown (default), wiki markup, storage format, or atlas_doc_format JSON. Use storage or atlas_doc_format when retransmitting existing page structure and render metadata. |
| `parent_id` | `string` | No | (Optional) parent page ID. If provided, this page will be created as a child of the specified page |
| `content_format` | `string` | No | (Optional) The format of the content parameter. Options: 'markdown' (default), 'wiki', 'storage', or 'atlas_doc_format'. Wiki format uses Confluence wiki markup syntax. Choose `storage` for Confluence macro/layout round-tripping and `atlas_doc_format` for Confluence Cloud ADF pages when smart-link, datasource, or embedded Jira display metadata must be preserved. |
...
<Tip>
Use Markdown for new prose. Use `content_format: "storage"` for Confluence storage XHTML and `content_format: "atlas_doc_format"` for Confluence Cloud ADF JSON when retransmitting existing page structure or render metadata. Specify `parent_id` to create as a child page.
</Tip>
...
| `content` | `string` | Yes | The new content of the page. Format depends on content_format parameter. Use storage or atlas_doc_format when retransmitting existing page structure and render metadata. |
...
<Tip>
Set `is_minor_edit: true` to skip notification emails. Use `content_format: "storage"` or `content_format: "atlas_doc_format"` when updating existing rich pages so Confluence render metadata is retransmitted unchanged.
</Tip>
